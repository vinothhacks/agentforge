...
from gateway.compiler import compile_pda_query, persist_spec
...
def main(argv: list[str] | None = None) -> int:
    ...
    p.add_argument("--samples", action="store_true", help="Write sample PDA .txt files into --dir.")
    p.add_argument("--experiment", action="store_true", help="Run P-1 disagreement experiment.")
    p.add_argument("--eval", action="store_true", dest="run_eval", help="Run offline PDA golden evals.")
    ...
    if args.samples or args.eval_fixtures:
        ...
        print(f"wrote sample PDAs in {workspace}")
        return 0

    if args.run_eval:
        if not any(workspace.glob("PDA-*")) and not any(workspace.glob("*.pdf")):
            write_sample_pdas(workspace)
    ...
    if args.ingest:
        ...
        if warning.get("no_text_layer_count"):
            print(
                f"WARNING: {warning['no_text_layer_count']} PDAs have no text layer "
    ...
    spec = compile_pda_query(workspace)
    ...
    print(f"Open {url}  — paste an OpenRouter key, ask about your PDAs.")
    print("Export to Excel comes next.")

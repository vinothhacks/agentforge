...
from scripts.build_advanced_corpus import write_advanced_pdas
...
def test_advanced_enumeration_and_precision(tmp_path: Path):
    dem = write_advanced_pdas(tmp_path, n=24, with_pdf=True)
...
def test_pdf_text_layer_extracts_amounts(tmp_path: Path):
    write_advanced_pdas(tmp_path, n=3, with_pdf=True)
